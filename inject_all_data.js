...
if (!data.includes('"pronouns":') && proStr.length > 50) {
...
let keigoStr = scriptKeigo.substring(scriptKeigo.indexOf('"keigoData":'), scriptKeigo.indexOf('  ]\n`') + 3);
if (!data.includes('"keigoData":') && keigoStr.length > 50) {
  data = data.replace('window.JLPT_DATA = {', 'window.JLPT_DATA = {\n  ' + keigoStr + ',');
}

fs.writeFileSync('c:/ai/jlpt-n5-textbook/data.js', data, 'utf8');
console.log('Injected pronouns, adverbs, keigoData');
