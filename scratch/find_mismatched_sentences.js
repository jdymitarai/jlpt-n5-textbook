...
// Helper to get conjugated forms of a verb
function getVerbForms(word, furigana) {
  if (!word || typeof word !== 'string') return [];
  const forms = [word];
  
  // Conjugate based on ending
  if (word.endsWith('する')) {
    ...
    // For五段 verbs ending in る (like 括る, 走る): stem changes to ら, り, る, れ, ろ, っ
    ...
    const root = word.slice(0, -1);
    forms.push(root + 'に', root + 'な', root + 'ん', root + 'ね', root + 'の');
...
// Check if example sentence contains the word or its forms
function checkSentence(word, furigana, sentence, conjugations) {
  ...
  // 2. Check stem / Kanji-only for kanji words
  const kanjiMatch = word.match(/^([\u4e00-\u9faf]+)/);
  if (kanjiMatch) {
    const kanjiPart = kanjiMatch[1];
    // If the kanji part is not in the sentence, it's definitely a mismatch
    if (!sentence.includes(kanjiPart)) return false;
  }

  // 3. For verbs/adjectives, check conjugations
...
console.log("Scanning databases for example sentence mismatches...\n");
...
levels.forEach(lvl => {
  ...
  console.log(`=== Level: ${key} (Total Words: ${data.vocabulary.length}) ===`);
  console.log(`Mismatches found: ${mismatches.length}`);
  mismatches.forEach(m => {
    console.log(`  - Word: "${m.word}" (${m.furigana}) | Meaning: "${m.meaning}"`);
    console.log(`    Sentence: "${m.exampleJa}"`);
    console.log(`    Translation: "${m.exampleEn}"`);
  });
  console.log();
});
