...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('data-subtab="keigo">敬語系統對照</button>')) {
    if (!lines[i+1].includes('data-subtab="tenses"')) {
      lines.splice(i + 1, 0, '            <button class="btn btn-secondary subtab-btn" data-subtab="tenses">四大詞性時態對照</button>');
      buttonInserted = true;
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('<div id="keigo-container"></div>')) {
    // The structure around here is:
    // 563: <div id="keigo-container"></div>
    // 564: </div>
    // 565: </div>
    // We want to insert the view right before line 565 (</div> that closes the subviews container)
    // Wait, let's look closely at the closing tags. 
    // 563: <div id="keigo-container"></div>
    // 564:             </div>
    // 565:           </div>
    // 566:         </section>
    // We want to insert right after 564.
    
    // Check if not already inserted
    if (!lines.slice(i, i+10).some(l => l.includes('subtab-tenses-view'))) {
      ...
      lines.splice(i + 2, 0, tensesViewHTML);
      viewInserted = true;
      break;
    }
  }
...
if (buttonInserted || viewInserted) {
  fs.writeFileSync('index.html', lines.join('\n'), 'utf8');
  console.log('Successfully patched index.html with line-based approach.');
} else {
  console.log('No patch needed or markers not found.');
}
