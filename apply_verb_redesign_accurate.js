...
const targetStr = fs.readFileSync('temp.js', 'utf8').trim();

// Note: Because of encoding issues with cat output, I will find the block using indexes of known substrings.

const startIdx = appJS.indexOf('// 1. Render Verbs');
const endIdx = appJS.indexOf('// 2. Render Adjectives');

if (startIdx !== -1 && endIdx !== -1) {
  const replacement = `// 1. Render Verbs
    ...
    if (tableVerbs) {
      ...
      levelsToRender.forEach(lvl => {
        ...
        verbConjugations.forEach((v, idx) => {
          ...
          const html = \`
            <tr style="border-bottom: 1px solid var(--border-color); cursor: pointer;" onclick="document.getElementById('conj-\${verbId}').classList.toggle('hidden');">
              <td style="vertical-align: middle;">\${getLvlBadge(lvl)}</td>
              <td style="line-height: 1.4; vertical-align: middle;">
                <div class="clickable-jp" data-speech="\${v.dictionary.split(" ")[0]}" style="font-size: 1.1em; font-weight: bold; color: var(--text-primary);">\${v.dictionary}</div>
                <div class="clickable-jp" data-speech="\${v.masu.split(" ")[0]}" style="font-size: 0.85em; opacity: 0.8; color: var(--text-secondary);">\${v.masu}</div>
              </td>
              ...
              <td colspan="6" style="padding: 12px;">
                <div style="display: grid; grid-template-columns: 80px 1fr 1fr; gap: 8px; font-size: 0.9em; text-align: left;">
                  ...
                  <div style="background: var(--bg-primary); padding: 8px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8em; margin-bottom: 4px;">常體 (て形 / ている)</div>
                  ...
                  <div style="background: var(--bg-primary); padding: 8px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8em; margin-bottom: 4px;">敬體 (ています)</div>
                    <div class="clickable-jp" data-speech="\${teImasu.split(" ")[0]}" style="color: #3B82F6; margin-top: 24px;">\${teImasu}</div>
                  </div>
                  ...
                  <div style="background: var(--bg-primary); padding: 8px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8em; margin-bottom: 4px;">常體 (た形)</div>
                    <div class="clickable-jp" data-speech="\${taForm.split(" ")[0]}" style="color: #10B981;">\${taForm}</div>
                  </div>
                  <div style="background: var(--bg-primary); padding: 8px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8em; margin-bottom: 4px;">敬體 (ました)</div>
                  ...
                  <div style="background: var(--bg-primary); padding: 8px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8em; margin-bottom: 4px;">常體 (ない / なかった)</div>
                  ...
                  <div style="background: var(--bg-primary); padding: 8px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div style="color: var(--text-secondary); font-size: 0.8em; margin-bottom: 4px;">敬體 (ません / ませんでした)</div>
      ...
      if (!hasData) tableVerbs.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 20px;">此分類目前無資料</td></tr>';
    }
  ...
  console.log('Successfully updated app.js using index boundaries.');
} else {
