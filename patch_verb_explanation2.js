...
const startMarker = '<div class="grammar-rule-card"';
const titleMarker = '日文動詞三大分類指南</h4>';
const endMarker = '<table class="consolidation-table">';

let searchStart = html.indexOf(titleMarker);
if (searchStart === -1) {
  console.log("Could not find title marker");
  process.exit(1);
}

// Find the <div class="grammar-rule-card" BEFORE the title
let startIdx = html.lastIndexOf(startMarker, searchStart);

// Find the <div style="overflow-x: auto;"> before the table AFTER the title
let tableIdx = html.indexOf(endMarker, searchStart);
let endIdx = html.lastIndexOf('<div style="overflow-x: auto;">', tableIdx);
